window.addEventListener("load", function() {
  ...
  function drawBackground() {
    ...
    // middle line
    bgCtx.moveTo(0, lensY);
    bgCtx.lineTo(bg.width, lensY);

    // lens
    bgCtx.moveTo(lensX - lensW / 2, lensY - lensH / 2);
    bgCtx.quadraticCurveTo(lensX, lensY, 
                           lensX - lensW / 2, lensY + lensH / 2);
    bgCtx.lineTo(lensX + lensW / 2, lensY + lensH / 2);
    bgCtx.quadraticCurveTo(lensX, lensY,
                           lensX + lensW / 2, lensY - lensH / 2);
    bgCtx.closePath();
...
*/
    ...
    if (showExtRay)
      drawLine(calcX(lensX - lensFocus, lensY, 
                     lensX, rayY, fg.height), fg.height, 
               lensX, rayY, true);

    // refracted parallel ray
    if ($("#refracted-parallel-ray").checked)
      drawLine(lensX, rayY, 
               fg.width, calcY(lensX - lensFocus, lensY, 
                               lensX, rayY, fg.width));
